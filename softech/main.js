...
// Inspired by Mitsumi Distribution Model
// Features 3D Consumer IT Goods Visualizer Studio & Responsive Top Bar
...
const translations = {
    en: {
        ...
        footer_rights: "© 2026 Soft Tech Distribution. All rights reserved. Inspired by Mitsumi Distribution model."
    },
    es: {
        ...
        about_desc: "Inspirados en la excelencia logística global de distribución tipo Mitsumi, conectamos fabricantes tecnológicos globales de laptops y movilidad con cadenas minoristas.",
    ...
    fr: {
        ...
        about_desc: "Inspiré par le modèle de distribution informatique Mitsumi, Soft Tech sert de pont stratégique pour les ordinateurs portables, tablettes et produits mobiles.",
    ...
    sw: {
        ...
        about_desc: "Ikiongozwa na mfano bora wa usambazaji wa Mitsumi Distribution, Soft Tech inaleta laptop, tablet, na vifaa vya kisasa katika maduka ya kikanda.",
...
/* ==========================================================================
   3. THREE.JS 3D CONSUMER IT GOODS VISUALIZER ENGINE
   ========================================================================== */
let scene, camera, renderer, currentMesh, animFrameId;
let autoRotate = true;
...
const productModelsData = {
    laptop: {
        name: "Ultra-Slim Executive Laptop (Book Pro 16\")",
        hotspotTitle: "16\" Organic OLED 120Hz Display",
        hotspotDesc: "Ultra-thin aluminium unibody laptop with anti-glare display and glass trackpad for commercial deployment.",
        specs: { proc: "M-Series Octa-Core", power: "18 Hours", carbon: "1.25 kg", rating: "Retail OEM Grade" }
    },
    tablet: {
        name: "Flex Tab Ultra (Foldable 5G Tablet)",
        hotspotTitle: "11\" Dynamic Foldable AMOLED",
        hotspotDesc: "Dual-display 5G tablet with pen stylus magnetic dock and triple AI camera array.",
        specs: { proc: "Snapdragon 8 Gen 3", power: "14 Hours", carbon: "480 grams", rating: "Enterprise Mobility" }
    },
    headset: {
        name: "Audio Pro Haptic Wireless Headset",
        hotspotTitle: "50mm Bio-Cellulose Drivers",
        hotspotDesc: "Active Noise Cancellation (ANC) headset with lossless 3D spatial audio and 60h battery life.",
        specs: { proc: "Haptic Spatial Audio", power: "60 Hours", carbon: "280 grams", rating: "High-End Audio" }
    }
};

let currentModelKey = 'laptop';
...
function init3DVisualizer() {
    ...
    if (typeof THREE === 'undefined') {
        container.innerHTML = `<div style="display:flex; align-items:center; justify-content:center; height:100%; color:#fff; font-weight:600;">3D Engine Loading...</div>`;
        return;
    }
    ...
    const dirLight1 = new THREE.DirectionalLight(0xB87D70, 1.3);
    dirLight1.position.set(5, 8, 5);
    scene.add(dirLight1);

    const dirLight2 = new THREE.DirectionalLight(0x405D41, 1.1);
    dirLight2.position.set(-5, -3, -5);
    scene.add(dirLight2);

    const pointLight = new THREE.PointLight(0xF7F4EE, 0.9, 10);
    pointLight.position.set(0, 2, 2);
    scene.add(pointLight);

    build3DMesh(currentModelKey);
    ...
    const btnWireframe = document.getElementById('btnWireframe3D');
    ...
    const btnReset = document.getElementById('btnResetCamera3D');
    btnReset?.addEventListener('click', () => {
        camera.position.set(0, 1.2, 4.2);
        if (currentMesh) {
            currentMesh.rotation.set(0.2, 0, 0);
        }
    });
...
function build3DMesh(modelKey) {
    ...
    const rosyBrownMat = new THREE.MeshStandardMaterial({
        color: 0xB87D70,
        roughness: 0.3,
        metalness: 0.6,
        wireframe: isWireframe
    ...
    const glassTrackpadMat = new THREE.MeshPhysicalMaterial({
        color: 0xB87D70,
        transmission: 0.5,
        opacity: 0.9,
        transparent: true,
    ...
    if (modelKey === 'laptop') {
        const baseGeom = new THREE.BoxGeometry(2.4, 0.08, 1.6);
        const baseMesh = new THREE.Mesh(baseGeom, darkGreenMat);
        baseMesh.position.set(0, -0.4, 0);
        group.add(baseMesh);

        const kbGeom = new THREE.BoxGeometry(2.0, 0.02, 0.8);
        const kbMesh = new THREE.Mesh(kbGeom, rosyBrownMat);
        kbMesh.position.set(0, -0.35, -0.2);
        group.add(kbMesh);

        const padGeom = new THREE.BoxGeometry(0.7, 0.02, 0.45);
        const padMesh = new THREE.Mesh(padGeom, glassTrackpadMat);
        padMesh.position.set(0, -0.35, 0.45);
        group.add(padMesh);

        const lidGroup = new THREE.Group();
        lidGroup.position.set(0, -0.36, -0.78);

        const screenFrameGeom = new THREE.BoxGeometry(2.4, 1.5, 0.05);
        const screenFrameMesh = new THREE.Mesh(screenFrameGeom, silverMetalMat);
        screenFrameMesh.position.set(0, 0.75, 0);
        lidGroup.add(screenFrameMesh);

        const screenGlassGeom = new THREE.PlaneGeometry(2.25, 1.35);
        const screenGlassMesh = new THREE.Mesh(screenGlassGeom, screenDisplayMat);
        screenGlassMesh.position.set(0, 0.75, 0.03);
        lidGroup.add(screenGlassMesh);
    ...
    } else if (modelKey === 'tablet') {
        const tabLeftGeom = new THREE.BoxGeometry(1.2, 1.8, 0.08);
        const tabLeft = new THREE.Mesh(tabLeftGeom, darkGreenMat);
        tabLeft.position.set(-0.62, 0, 0);
        tabLeft.rotation.y = 0.15;
        group.add(tabLeft);

        const tabRightGeom = new THREE.BoxGeometry(1.2, 1.8, 0.08);
        const tabRight = new THREE.Mesh(tabRightGeom, rosyBrownMat);
        tabRight.position.set(0.62, 0, 0);
        tabRight.rotation.y = -0.15;
        group.add(tabRight);

        const dispLeftGeom = new THREE.PlaneGeometry(1.1, 1.7);
        const dispLeft = new THREE.Mesh(dispLeftGeom, screenDisplayMat);
        dispLeft.position.set(-0.62, 0, 0.05);
        dispLeft.rotation.y = 0.15;
        group.add(dispLeft);

        const dispRightGeom = new THREE.PlaneGeometry(1.1, 1.7);
        const dispRight = new THREE.Mesh(dispRightGeom, screenDisplayMat);
        dispRight.position.set(0.62, 0, 0.05);
        dispRight.rotation.y = -0.15;
        group.add(dispRight);

        const camGeom = new THREE.CylinderGeometry(0.12, 0.12, 0.06, 16);
        const camMesh = new THREE.Mesh(camGeom, silverMetalMat);
        camMesh.rotation.x = Math.PI / 2;
        camMesh.position.set(-1.0, 0.6, -0.06);
        group.add(camMesh);

    } else if (modelKey === 'headset') {
        const bandGeom = new THREE.TorusGeometry(1.0, 0.08, 16, 64, Math.PI);
        const bandMesh = new THREE.Mesh(bandGeom, darkGreenMat);
        bandMesh.position.set(0, 0.2, 0);
        group.add(bandMesh);

        const cupLeftGeom = new THREE.CylinderGeometry(0.45, 0.45, 0.35, 32);
        const cupLeft = new THREE.Mesh(cupLeftGeom, rosyBrownMat);
        cupLeft.rotation.z = Math.PI / 2;
        cupLeft.position.set(-1.0, -0.2, 0);
        group.add(cupLeft);

        const cupRight = new THREE.Mesh(cupLeftGeom, rosyBrownMat);
        cupRight.rotation.z = Math.PI / 2;
        cupRight.position.set(1.0, -0.2, 0);
        group.add(cupRight);

        const ringGeom = new THREE.TorusGeometry(0.46, 0.03, 16, 32);
        const ring1 = new THREE.Mesh(ringGeom, silverMetalMat);
        ring1.rotation.y = Math.PI / 2;
        ring1.position.set(-1.18, -0.2, 0);
        group.add(ring1);

        const ring2 = new THREE.Mesh(ringGeom, silverMetalMat);
        ring2.rotation.y = Math.PI / 2;
        ring2.position.set(1.18, -0.2, 0);
        group.add(ring2);
    }
...
function animate3D() {
    ...
    if (currentMesh && autoRotate && !isDragging) {
...
function updateSpecsUI(modelKey) {
    const data = productModelsData[modelKey];
    if (!data) return;

    const titleEl = document.getElementById('hotspotTitle');
    const descEl = document.getElementById('hotspotDesc');
    const procEl = document.getElementById('specChip');
    const powerEl = document.getElementById('specPower');
    const carbonEl = document.getElementById('specCarbon');
    const ratingEl = document.getElementById('specRating');

    if (titleEl) titleEl.textContent = data.hotspotTitle;
    if (descEl) descEl.textContent = data.hotspotDesc;
    if (procEl) procEl.textContent = data.specs.proc;
    if (powerEl) powerEl.textContent = data.specs.power;
    if (carbonEl) carbonEl.textContent = data.specs.carbon;
    if (ratingEl) ratingEl.textContent = data.specs.rating;
}
...
function initModals() {
    ...
    btnCloseService?.addEventListener('click', () => serviceModal?.classList.remove('active'));
...
window.openServiceModal = function(serviceKey) {
    ...
    if (modalContent && serviceModal) {
        modalContent.innerHTML = `
            ...
            <button class="btn btn-rosy btn-full" onclick="document.getElementById('serviceModal').classList.remove('active'); document.getElementById('contactModal').classList.add('active');">
